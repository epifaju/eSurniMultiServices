...
public class SearchController {
        ...
        if (!q.isEmpty()) {
            artisans = artisans.stream()
                    .filter(a -> (a.getUser() != null && a.getUser().getName() != null
                            && a.getUser().getName().toLowerCase().contains(q.toLowerCase())) ||
                            (a.getCity() != null && a.getCity().toLowerCase().contains(q.toLowerCase())) ||
                            (a.getCategory() != null && a.getCategory().toLowerCase().contains(q.toLowerCase())))
                    .toList();
        }
        // Pagination
        int start = Math.min(page * size, artisans.size());
        int end = Math.min(start + size, artisans.size());
        List<ArtisanWithRatingDTO> content = artisans.subList(start, end).stream().map(a -> new ArtisanWithRatingDTO(
                a.getId(),
                ...
                a.getCity(),
                a.getCategory(),
                artisanService.getAverageRating(a))).toList();
    ...
    public Page<Annonce> searchAnnonces(
            ...
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        List<Annonce> annonces = annonceService.findByCityAndCategory(city, category);
        // Recherche par mot-clé sur titre, description, ville, catégorie
        if (!q.isEmpty()) {
            annonces = annonces.stream()
                    .filter(a -> (a.getTitle() != null && a.getTitle().toLowerCase().contains(q.toLowerCase())) ||
                            (a.getDescription() != null && a.getDescription().toLowerCase().contains(q.toLowerCase()))
                            ||
                            (a.getArtisan() != null && ((a.getArtisan().getCity() != null
                                    && a.getArtisan().getCity().toLowerCase().contains(q.toLowerCase())) ||
                                    (a.getArtisan().getCategory() != null
                                            && a.getArtisan().getCategory().toLowerCase().contains(q.toLowerCase())))))
                    .toList();
        }
        // Pagination
